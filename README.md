# Микросервис Ротация баннеров

Поднятие проекта в docker:
1. Выполнить команду: **make docker-up** (команда поднимет приложение и ее зависимости и накатит миграции).
2. Для запуска HTTP сервера необходимо в deployments/docker-compose.yaml в env: SERVER_ISHTTP проставить true, иначе запустится gRPC сервер.
3. Запросы с postman для HTTP сервера - Rotation Banner-HTTP.postman_collection.json.
